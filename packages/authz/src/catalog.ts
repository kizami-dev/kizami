 ...
 *
 * docs/design/permission-catalog.md §1(業務タスク単位の権限カタログ、34項目)をそのまま
...
export interface PermissionCatalogEntry {
  ...
  /** 編集UIで選択可能なスコープ(意味のあるものだけ。このカタログに "self" は登場しない)。 */
...
export const PERMISSION_CATALOG: readonly PermissionCatalogEntry[] = [
  ...
  // 1.x シフト(2026-08-23 v0.7 追加)。シフト表は「予定」の作成・確定であり、打刻の代理登録
  // (実績)や修正申請の承認とは業務タスクが異なるため独立項目にする — 「非エンジニアが
  // 業務タスク単位で理解できるカタログ」という設計原則(要件 §4)に従う。
  entry(
    "shift.manage",
    "シフト表を作成・確定できる",
    "メンバーのシフト表(勤務日・勤務時間・休日)を作成し、確定後の変更履歴を残しながら修正できる。変形労働時間制の「事前特定」に相当する操作",
    DEPT_SCOPES,
    false,
  ),
  ...
  // 1.8b 退職者の個人データ消去(2026-08-27 追加、docs/design/data-retention.md)。
  //
  // 判断点: `member.deactivate` の転用ではなく専用キーを新設した。2FA リセットのときは
  // 「他人のログイン要件を一段弱める操作は退職処理と同格」として転用したが、消去は同格ではなく
  // **一段重い**。無効化は取り消せる(再有効化がある)のに対し、消去は氏名・メール・認証情報を
  // 復元不能に失わせる終端状態で、実行後に「やり直す」経路が製品にもDBにも存在しない。
  // 退職処理を任せられる担当者と、個人データを不可逆に消してよい担当者は同じとは限らない。
  //
  // スコープはテナント全体のみ(TENANT_ONLY)。部署スコープを許すと「自部署の退職者だけ消せる
  // 部署長」が生まれるが、消去可否の判定材料(保持期間・法定帳簿の保存義務)は全社で1つであり、
  // 部署ごとに判断が分かれる余地が無い。個人情報保護法上の管理責任も事業者単位で負う。
  entry(
    "member.erase",
    "退職者の個人データを消去できる",
    "保持期間(労働基準法109条)が経過した退職者について、氏名・メールアドレス・認証情報などの個人データを消去(匿名化)できる。勤怠記録そのものは残る。取り消しできない操作",
    TENANT_ONLY,
    true,
  ),
  ...
  // 1.15 承認フロー設定(多段承認)。2026-08-24 追加、docs/design/approval-flows.md。
  // 判断点: 既存キーの転用を検討したが、いずれも意味が合わないため専用キーを新設した。
  // - `permission.preset.manage`: 「誰が何をできるか」の設定であって「承認が何段必要か」の
  //   設定ではない(段数を変えても誰の権限も増減しない)。
  // - `tenant_settings.calendar.manage` 等の `tenant_settings.*`: これらは**集計の入力**に
  //   なるテナント設定(日界・法定休日・フレックス・GPS)を指すグループで、承認フローは
  //   集計に一切影響しない。同じチェックボックスに束ねると「カレンダー担当者が承認の
  //   段数を変えられる」という説明しづらい状態になる。
  // - `attendance.correction.approve` / `leave.request.approve`: 承認する権限であって
  //   承認フローを決める権限ではない(承認者が自分の承認を1段に減らせてはいけない)。
  // 危険フラグは立てない: 段数の変更は**承認を厳しくする/緩める**方向の運用判断であって、
  // 権限の付与・個人情報の露出・データの不可逆な変更のいずれにも当たらない。2→1 に緩める
  // 変更も、仕掛かり中の申請には影響しない(作成時の段数を凍結する — 上記設計ドキュメント)。
  entry(
    "approval_flow.manage",
    "承認フロー(多段承認)を設定できる",
    "打刻修正・休暇・休憩自動控除打ち消しの各申請について、承認を1段(単段)にするか2段(一次承認+人事等による二次承認)にするかをテナント単位で設定できる",
    TENANT_ONLY,
    false,
  ),
];
