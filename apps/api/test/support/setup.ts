...
import { seedTenant, type SeededTenant } from "./seed.js";

// ランタイム非依存な中身は seed.ts 側にある(workerd スモークテストと共有するため)。
// 既存テストの import 元を変えずに済むよう、ここから再エクスポートする
export { extractCookie, jstMinutes, loginAndGetCookie, seedTenant } from "./seed.js";
export type { SeededTenant } from "./seed.js";
...
/**
 * テナント1件・ユーザー1件を投入した使い捨ての DB を用意する。
 *
...
export async function setupTestDb(existingDb?: Database): Promise<SeededTenant> {
  return seedTenant(existingDb ?? (await createTestDatabase()));
}
