 ...
 *
 * test/workers/ は workerd(Cloudflare Workers)専用のスモークで、`cloudflare:test` を
 * import するため Node では読み込めない。そちらは `pnpm test:workers`
 * (apps/api/vitest.workers.config.ts)が走らせるので、ここでは除外する。
 */
