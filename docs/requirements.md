...
**決定**: **清算期間1ヶ月・コアタイムなし**(スーパーフレックス)を既定とする。月の総枠(法定: 週平均40h×暦日数/7)との差分で時間外を判定。3ヶ月清算はロードマップ。

**追記**(2026-08-24, コアタイム実装): コアタイム(労基法32条の3の任意設定)に対応した。既定は従来どおり「なし」で、設定した場合もコアタイム中の不在は**警告(コアタイム遅刻・早退・不在)としてのみ**扱い、集計(清算期間の総枠)には影響しない — 賃金控除は給与側の責任という既存の原則([design/breaks.md](./design/breaks.md))を踏襲する。詳細は [design/work-systems.md](./design/work-systems.md)「コアタイム」。
...
- 打刻には証跡メタデータ(IPアドレス・UA・打刻手段)を添付。**GPS座標の取得は既定OFF** — テナント設定でopt-in。有効化時は従業員に取得中であることを明示する。保持期間の既定は勤怠記録本体と同一とし、テナント(会社)単位で短縮設定できる。なお退職者の個人データ消去(§7)を実行すると、この証跡メタデータ(IP・UA・GPS)は打刻の行を残したまま null 化される — 打刻の**時刻**は賃金台帳の基礎資料だが「どのIP・どの端末・どの座標から打刻したか」は保存義務の対象ではないため([design/data-retention.md](./design/data-retention.md))
...
**決定**: 打刻の事後修正は**本人直接編集不可**。修正申請→承認者のapproveで反映し、全変更を**不可変監査ログ**に記録する。承認フローは既定で単段。**多段承認(二段)はテナント設定で種別ごとに opt-in**(2026-08-24 実装 — 打刻修正・休暇・休憩自動控除打ち消しの3種別。一次承認=従来どおりスコープ内の承認権限保持者、二次承認=同じ権限をテナント全体スコープで持つ人。申請は作成時点の段数で最後まで進む — [design/approval-flows.md](./design/approval-flows.md))。
...
- **拒否(deny)ルール**(2026-08-24 実装。当初は「denyなし・加算のみで開始、deny対応はロードマップ」だった): 実効権限は「全プリセットの付与の合集合 − 全プリセットの拒否の合集合」。**拒否はどの付与にも優先**し、**スコープを持たない**(拒否したら全スコープで拒否 — 部分スコープの拒否は「部署では拒否だが全社では許可」という読み解きの罠を生むため設けない)。セルフサービス権限は拒否できない。詳細は [design/permission-catalog.md](./design/permission-catalog.md#拒否-deny-ルール)
- **操作は閲覧を含意**: 操作権限をONにすると必要な閲覧権限が自動的に付随。UI上も「この権限には○○の閲覧が含まれます」と明示し、「承認できるのに見えない」矛盾設定を作れなくする
- **固定原則(設定で変更不可)**: 本人の打刻・申請・自分の記録閲覧のセルフサービス権限は全員が常に保持(拒否ルールでも取り上げられない)。最後の「権限管理」保持者は削除・降格不可(割当変更だけでなく、拒否を足すプリセット編集も同じ判定で拒否する)。自分自身の権限昇格につながる操作は不可。プリセットの編集・割当変更はすべて監査ログ対象
...
- **比例付与(パート向け)対応**(2026-08-24 実装、労基法39条3項・労基法施行規則24条の3): メンバーごとに「有給付与の区分」(通常/週4日/週3日/週2日/週1日)を持ち、区分に応じた日数表で法定付与・付与の予告を計算する。区分は**管理者が就業規則・雇用契約に基づいて明示的に選ぶ**(週所定労働時間・日数からの自動判定はしない)。設計・判断点は [design/leave-proportional-grant.md](./design/leave-proportional-grant.md)
...
- **マルチテナント対応**。全データモデルに最初から `tenant_id` を設計。v1.0(2026-08-24)で有効化 — 1インスタンスに複数テナント(会社)を同居させられる。テナントの作成は**運用者の作業**(`pnpm --filter @kizami/api create-tenant`、[deploy/k8s/README.md](https://github.com/kizami-dev/kizami/blob/main/deploy/k8s/README.md) 参照)で、セルフサインアップ(自己申込みによるテナント作成)は提供しない — SaaS 形態での自己申込みはロードマップ。テナント間分離の実装規約と監査の範囲は [design/multi-tenancy.md](./design/multi-tenancy.md)
- 認証は**自前認証(email+パスワード、将来パスキー)とOIDC(Google Workspace / Entra ID等)の両対応**。自前はv0.1、OIDCはv1.0(2026-08-24 実装 — [design/sso-oidc.md](./design/sso-oidc.md))
- **OIDC(SSO)は「新規登録の入口」ではなく「既存ユーザーのログイン手段」**(2026-08-24 決定)。自動プロビジョニング(JIT)は行わない — コールバックでIdPが返したメールアドレスを、設定されたテナントの `users.email` と突合し、一致するユーザーが居なければログインを拒否する(「このメールアドレスのユーザーが見つかりません。管理者に招待を依頼してください」)。IdPに居ることは「この会社の従業員として勤怠を打つ人」を意味せず(業務委託・グループ会社・退職後の残存アカウント)、誰を登録するかは下記「登録は招待式のみ」と同じく管理側の決定事項のため。設定はテナント単位(issuer / client_id / client_secret〔保存時暗号化〕/ 有効フラグ / 未検証メール許可フラグ〔既定OFF〕)で、権限は `tenant_settings.auth.manage`。パスワードログインは併存する
- **二要素認証(TOTP)は自前認証の任意オプション**(2026-08-27 実装 — [design/two-factor-auth.md](./design/two-factor-auth.md))。従業員本人が「設定 → セキュリティ」(`/settings/security`)で有効化する(テナントによる強制は将来課題)。方式は RFC 6238(30秒・6桁・SHA-1・±1ステップ許容)で、認証アプリ(Google Authenticator 等)を使う。実装は依存を増やさず自前(WebCrypto のみ・RFC のテストベクタで固定)。共有鍵は `KIZAMI_ENCRYPTION_KEY` で暗号化して保存し、**鍵が無い配備では 2FA を使えない**(平文フォールバックはしない)。端末紛失に備えて単回使用のリカバリコード10本を発行し、それも失った場合は `member.deactivate` 権限(退職処理と同格)を持つ管理者がリセットできる(監査ログ + 本人へアプリ内通知)。**SSO ログインは 2FA をバイパスする** — 多要素を課すかは IdP 側のポリシーが決めるため
...
- **退職者データの保持と消去**(2026-08-27 実装 — [design/data-retention.md](./design/data-retention.md))。労働基準法109条の保存義務(原則5年・附則143条2項の経過措置により当分の間3年)と、個人情報保護法22条の「利用する必要がなくなった個人データの遅滞なき消去」(努力義務)は退職者について正面から衝突する。KIZAMI はこれを **「保持期間の経過後に、行は残したまま匿名化する」** という形で解く。①退職処理(`member.deactivate`)が `users.deactivated_at` を記録し、これが保持期間の起算日になる(退職処理は最終出勤日以降に行われるため、義務期間より長く保持する方向にしか倒れない)。②保持年数はテナント設定(3 or 5、**既定5** — 5年が原則で3年は経過措置にすぎず、既定を3にすると経過措置終了時に全テナントが一斉に違反側へ倒れるため)。③期間の経過後、専用権限 `member.erase`(**新設**。テナント全体スコープ・危険フラグ付き。`member.deactivate` の流用にしない — 無効化は再有効化で取り消せるが消去には戻す経路が無く、同格ではなく一段重い)を持つ管理者が消去できる。④**消去 = 匿名化**であり行削除ではない: 氏名は「削除済みユーザー」、メールは tombstone(`user_deleted_<id>@invalid`)に置き換え、パスワード・2FA・セッション・購読・個人通知設定・APIキー・招待/リセットトークン・Slack連携は物理削除、`punch_events` の IP・UA・GPS 列は null 化する一方、**勤怠記録・締めスナップショット・監査ログの行は1件も消さない**。⑤消去済みは無効化とは別の**終端状態**で、再有効化できない。⑥確認ダイアログでは影響の列挙に加えて対象者氏名の再入力を求める。⑦日次ワーカーによる通知はしない(消去は期限のある義務ではなく「もう消してよい」という状態にすぎず、急かすと取り返しのつかない誤操作を誘う)。**テナント自体の削除はスコープ外**(SaaS 形態のフェーズで扱う)
...
- 通知チャネル: **メール(SMTP)・Slack/Discord Webhook・Web Push/アプリ内**の3系統(フェーズ分割は§11)。用途は打刻忘れリマインド・承認依頼・年5日/36協定アラート等。Web Push は 2026-08-24 実装(VAPID を自前実装し外部サービスに依存しない。設計は [design/web-push.md](./design/web-push.md))
...
| 個人の受け取り設定 | 従業員本人 | 通知の種類ごとにアプリ内 / メール / ブラウザプッシュの ON/OFF、個人 Webhook、通知先メールアドレス、購読したブラウザ |

ブラウザプッシュ通知(Web Push、2026-08-24 追加)は**個人チャネルにのみ存在する** —
宛先が「特定個人のブラウザ」である以上、テナント共有チャネルとしては意味を成さないため。
運用者が VAPID 鍵(`VAPID_PUBLIC_KEY` / `VAPID_PRIVATE_KEY` / `VAPID_SUBJECT`)を設定した
配備でのみ有効になり、未設定なら UI からもチャネルからも消える([design/web-push.md](./design/web-push.md))。

**通知の宛先の原則**:

- **本人宛の通知**(打刻忘れ・有給失効間近・年5日期限・自分の申請の承認結果・
  自分の勤務とシフトとの乖離)は**本人の個人チャネルにのみ**送る。
  テナント共有 Webhook には送らない
