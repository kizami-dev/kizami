...
- **マルチテナント対応**。全データモデルに最初から `tenant_id` を設計。v1.0(2026-08-24)で有効化 — 1インスタンスに複数テナント(会社)を同居させられる。テナントの作成は**運用者の作業**(`pnpm --filter @kizami/api create-tenant`、[deploy/k8s/README.md](https://github.com/sasagar/kizami/blob/main/deploy/k8s/README.md) 参照)で、セルフサインアップ(自己申込みによるテナント作成)は提供しない — SaaS 形態での自己申込みはロードマップ。テナント間分離の実装規約と監査の範囲は [design/multi-tenancy.md](./design/multi-tenancy.md)
