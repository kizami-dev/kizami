...
| 以降 | ロードマップ | シフト制/変形労働、コアタイム、3ヶ月清算、freee/MF連携、比例付与、**有給付与の予告→承認→通知フロー**(2026-08-23 決定: (1) 付与基準日の事前に管理者へ予告通知 — 対象者・算定日数に加え、打刻データから計算した**出勤率の参考値**(労基法39条1項の8割出勤要件の検算材料)を添える。(2) 管理者が確認のうえ付与を実行(または予定された付与を承認)。(3) 実行時に本人へ「◯日付与されました」を leave_alert カテゴリ・個人チャネルで通知。機械が無条件に付与を確定させない — 出勤率要件の最終判断は人が行う。シフト制対応と同時期に実施)、Web Push、多段承認、権限denyルール、オンボーディングツアー |
