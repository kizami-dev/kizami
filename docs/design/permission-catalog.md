...
- 前提:
  ...
  - **拒否(deny)はプリセットが持つ**(2026-08-24 実装。それ以前は「denyは存在しない・加算のみ」だった)。付与は加算(union)され、そこから拒否が差し引かれる。詳細は本文書 [§拒否(deny)ルール](#拒否-deny-ルール)。
...
| `leave.grant.manage` | 有給休暇の付与・残日数を管理できる | 法定基準日方式等に基づく有給付与の実行、残日数の個別調整、**付与予告の承認・却下**(v0.7)ができる | 自部署+配下部署 / テナント全体 | 有給残日数閲覧(`leave.balance.view`相当) | **はい** |
...
- ステータス: 実装済み(2026-08-24)。[要件定義書](../requirements.md) §4 のロードマップ項目「権限denyルール」に対応する
- 機械可読な実体: `packages/authz/src/resolve.ts` の `resolveEffectivePermissions()` / `applyDenies()`、保存先は `permission_presets.denies` 列

### 合成規則

実効権限 = **(割り当てられた全プリセットの grants の union) − (割り当てられた全プリセットの denies の union)**。

拒否は**どのプリセットの付与にも優先する**。あるプリセットが `closing.execute` をテナント全体で付与していても、同じユーザーに割り当てられた別のプリセットが1つでも `closing.execute` を拒否していれば、そのユーザーは締めを実行できない。プリセットの割当順・作成順は結果に影響しない(順序に依存しないので「どの順で割り当てたか」を運用が気にする必要がない)。

### 拒否はスコープを持たない(全面的)

grants は権限キー×スコープの組だが、**denies は権限キーだけ**で、拒否は常に全スコープに及ぶ。

これは意図的な制限である。部分スコープの deny(例:「自部署では拒否、テナント全体では許可」)を許すと、「スコープが広がるほど行使できる範囲も広がる」という本モデルの前提が反転し、**「部署では拒否だが全社では許可」という読み解きの罠**が生まれる。実際、そのような設定を見た担当者が「この人は自部署のメンバーには何もできないが、他部署も含めた全社に対してならできる」という結論に辿り着くのは難しい。拒否は「その業務タスクを一切させない」という全面的な意思表示に限定し、細かい線引きは**付与する側のスコープ**で表現する。

### 拒否と「操作は閲覧を含意する」の関係

拒否は含意の展開の**前後どちらにも**効く。

- 拒否された操作権限は、その**含意先の閲覧権限も生まない**。`closing.execute` を拒否すれば、他に付与元がない限り `closing.view` も得られない(拒否したのに、拒否した権限の道連れで得ていた閲覧だけが残る、という中途半端な状態を作らない)
- 他の権限の含意によって、**拒否された閲覧権限が復活することはない**。`attendance.record.view` を拒否すれば、`shift.manage` がそれを含意していても得られない

### 拒否できない権限(固定原則)

**セルフサービス権限**(自分の打刻・自分の申請の起票・自分の記録の閲覧)は拒否の対象外である。評価器は該当キーへの deny を黙って無視し(`packages/authz/src/self-service.ts` の `UNDENIABLE_PERMISSIONS`)、API も本カタログに存在しないキーの deny を 400 `invalid_denies` で弾く。自分の打刻ができないユーザーを作れてしまうと、そのユーザーは労働時間を記録する手段を失う(=使用者が労働時間把握義務を果たせない)。

### 最後の権限管理保持者の保護

拒否の導入により、**割当を一切触らずに**テナントから権限管理者を消せるようになった(プリセットに `permission.preset.manage` の deny を1つ足すだけでよい)。そこで固定原則「最後の権限管理保持者の保護」を割当変更(`PUT /members/:id/presets`)だけでなく**プリセット編集(`PATCH /presets/:id`)にも適用する**。保存前に、編集後の内容でテナント全ユーザーの実効権限を計算し直し、`permission.preset.manage` の保持者が0人になる編集は 409 `last_admin` で拒否する(grants を削る編集でも同様)。

### 編集UI

権限プリセット編集画面では、拒否は**独立した「拒否(deny)」セクション**として付与のチェックボックス群とは別に置く(各項目を「付与/未設定/拒否」の3状態トグルにはしない)。3状態トグルはチェックボックス1個の見た目に意味が3つ乗るため、権限設計に慣れていない担当者が「未設定」と「拒否」を取り違える。セクションには常時「拒否はすべての付与に優先します。別のプリセットで付与されていても無効になります」の警告を添える。

実効権限ビュー(「このメンバーができること」)では、付与と拒否が衝突している項目を一覧から消さず、**取り消し線+「拒否」チップ+拒否元プリセット名**で表示する。付与元が1つもない権限は元から一覧に出ないので、この表示が出るのは管理者が最も理由を知りたい「衝突している」ときだけになる。
