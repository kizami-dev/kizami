...
既存の暗号化済み秘密情報は復号できなくなる — 通知チャネル・SSO の client secret については
該当機能が自動的に無効化されるだけで済む設計だが、**二要素認証(2FA)だけは例外で、
2FA を有効にしている利用者はログインできなくなる**(503)。共有鍵を復号できない状態で
2FA を素通りさせると「鍵を消せば 2FA が外れる」ことになるため、意図的にログインを止めている。
その場合の復旧手段は `member.deactivate` 権限を持つ別の管理者による 2FA リセット
(docs/design/two-factor-auth.md)。鍵をローテーションする際は、2FA 利用者に
事前の再設定を依頼すること):
